...
class solicitudesController extends Controller
{
	// SOLICITUD DE COTIZACION
			...
			for($i=0;$i<count($req -> productos);$i++ ):
				...
				$movimiento = DB::table('movim_sol')->insertGetId(
					[
							...
							'descripcion'   => $req -> productos[$i]['descripcion'],
							'id_compromiso' => $req -> id_compromiso,
							'id_cliente'    => $req -> id_cliente,
							'tipo'					=> 2   // SOLICITUD DE COTIZACION
					]
				);
			endfor;
			$this -> actualiza_estatus_compromiso($req -> id_compromiso, $req -> estatus);
			...
			for($i=0;$i<count($req -> productos);$i++ ):
				...
				if($req -> productos[$i]['id'] === null):
					...
					DB::table('movim_sol')->insertGetId(
						...
						$this -> actualiza_producto($req -> productos[$i]['id_producto'], $req -> productos[$i]['descripcion']);
				endif;
		...
		public function generar_solicitud_ft(Request $req){
			...
			$fichaTecnica = DB::table('movim_sol')->insertGetId(
				[
						...
						'id_compromiso' => $req -> id_compromiso,
						'id_cliente'    => $req -> id_cliente,
						'tipo_prod'     => $req -> tipo_prod,       // 1=EXISTENTE , 2=NUEVO
						'tipo'					=> 3   // SOLICITUD DE FT   // 1=SOL.PEDIDO, 2=SOL.COT, 3=SOL.FT
				]
			);
			$this -> validaEstatusCompromiso($req);
			return $fichaTecnica ? response('La solicitud se creo correctamente.',200) : 
														 response('Ocurrio un error intentelo nuevamente.',500); 
		}


		public function crear_nuevo_producto($producto, $fecha, $id_cliente){
			return DB::table('prodxcli')->insertGetId(
							[
		...
		public function actualiza_producto($id_producto, $descripcion){
			DB::update('UPDATE prodxcli SET descripcion=:descripcion 
												WHERE id=:id', [ 'descripcion' => $descripcion, 
																				 'id'				   => $id_producto]);
		}
		public function actualiza_estatus_compromiso($id, $estatus){
			DB::update('UPDATE compromisos SET estatus=:estatus WHERE id=:id', [ 'estatus' => $estatus, 'id'=> $id]);
		}
		public function obten_productos_x_compromiso($id){
			return DB::select('SELECT m.*, p.codigo FROM movim_sol m LEFT JOIN prodxcli p ON m.id_producto = p.id 
														WHERE id_compromiso = ?',[$id]);
		}
		public function cancelar_prod_a_cotizar(Request $req){
		 	$cancelar = DB::update('UPDATE movim_sol SET estatus=4, cancelado=:cancelado WHERE id=:id', 
			 													['cancelado' => $req -> fecha, 
																 'id'        => $req -> id]);
			
			$this -> validaEstatusCompromiso($req);
			return $cancelar ? response('Cancelado correctamente',200): response('Ocurrio un error, intentelo nuevamente');
		}
	...
	//! CONSULTAR DETALLE DE LA SOLICITUD
		...
		public function addSolicitud(Request $req){
			...
			if(!$detalle):
				$this -> actualizaCompromisos($fecha,$hora,$id_compromiso,$id_solicitud);														// !ACTUALIZO COMPROMISO
	...
	// TODO ****************************************************************************************
		...
		public function actualizaCompromisos($fecha,$hora,$id,$id_solicitud){
			$actualizaEstatus = DB::update('UPDATE compromisos SET cumplimiento=:cumplimiento, fecha_cierre=:fecha_cierre, hora_cierre=:hora_cierre, obs_usuario=:obs_usuario
																				WHERE id=:id', [ 'cumplimiento' => 1, 

																												'fecha_cierre' => $fecha, 
																												'hora_cierre'  => $hora, 
																												'obs_usuario'  => 'SE GENERÓ UNA SOLICITUD DE PEDIDO NÚMERO'.' '.$id_solicitud,	
																												'id' 					=> $id 
																												]);
		}
	...
	// TODO ********************************************************************************************
		...
		public function PendientesxValidar(Request $req){
			$pendientes = DB::select('SELECT m.id, m.id_depto, m.id_producto, p.codigo, p.dx, m.id_cliente, m.id_compromiso, 
																			 c.nombre as nomcli, m.fecha, m.tipo, m.tipo_prod, m.id_encargado, m.estatus
																	FROM movim_sol m LEFT JOIN prodxcli p ON m.id_producto = p.id
																									 LEFT JOIN clientes c ON m.id_cliente  = c.id
																WHERE m.estatus = ? AND m.id_creador = ?', [$req -> estatus , $req -> id_usuario]);
			return $pendientes ? $pendientes:[];


		}

		public function consulta_cot_autorizadas(Request $req){
			$autorizadas = DB::select('SELECT m.id, m.id_depto, m.id_producto, p.codigo, p.dx, m.id_cliente, m.id_compromiso,
																			 c.nombre as nomcli, m.fecha, m.tipo, m.tipo_prod, m.id_encargado, m.estatus
																	FROM movim_sol m LEFT JOIN prodxcli p ON m.id_producto = p.id
																									 LEFT JOIN clientes c ON m.id_cliente  = c.id
																WHERE m.id_creador = ? AND m.autorizacion = ? AND m.estatus = 3 OR m.estatus = 5', 
																[ $req -> id_usuario, $req -> fecha]);
		...
		public function validaEstatusCompromiso($data){
			$cero =0;$uno=0 ; $dos=0 ; $tres=0 ; $cuatro=0; $cinco=0; $movim=[];

			$movim = DB::select('SELECT * FROM movim_sol WHERE id_compromiso=? ', [$data -> id_compromiso]);

			if(count($movim) === 0){
				$objetTemp  = ["id" => $data['id_compromiso'], "estatus" => 1 ];
				$this -> actualizaEstatusCompromiso($objetTemp);
			}
				...
				elseif($dos > 0):
					$objetTemp  = ["id" => $data['id_compromiso'], "estatus" => 1 ];
					 $this -> actualizaEstatusCompromiso($objetTemp);
				elseif($tres > 0):
					$objetTemp  = ["id" => $data['id_compromiso'], "estatus" => 3 ];
					 $this -> actualizaEstatusCompromiso($objetTemp);
				elseif($cuatro > 0):
					$objetTemp  = ["id" => $data['id_compromiso'], "estatus" => 0 ];
					 $this -> actualizaEstatusCompromiso($objetTemp);
				elseif($cinco > 0):
					$objetTemp  = ["id" => $data['id_compromiso'], "estatus" => 3 ];
					$this -> actualizaEstatusCompromiso($objetTemp);
		...
		public function actualizaEstatusCompromiso($data){
		
			DB::update('UPDATE compromisos SET estatus=:estatus WHERE id=:id',
									['estatus' => $data['estatus'], 'id'=> $data['id'] ]);
		}
